...
Android 多模块工程：在可复用 Kotlin MVVM 脚手架之上落地了真实三模块 App——**新闻**（聚合数据接口：分类列表 + 分页 + 详情 WebView）、**天气**（聚合数据：实况+近5天预报+生活指数，天气卡随天气切实景照片背景，默认杭州可切换城市，DataStore 持久化）、**我的**（本地 Room 账号：注册/登录/退出，DataStore 持久登录态）。单 Activity + BottomNavigationView 三 Tab，每个业务是独立 feature module，自带 navigation graph，app 只做壳。
...
- **module_news** (`com.btg.news`)：新闻。聚合数据接口（`JuheResponse.unwrap` → safeApiCall 体系，`@NewsRetrofit` 独立 baseUrl `https://v.juhe.cn/`）；API key 走 `local.properties` 的 `JUHE_NEWS_API_KEY` → BuildConfig，不入库；TabLayout 10 分类 + 分页 + SwipeRefresh + StateLayout + 详情 WebView。
- **module_weather** (`com.btg.weather`)：天气。聚合数据接口（独立 `@WeatherJuheRetrofit`，baseUrl `https://apis.juhe.cn/`）；**天气与新闻是聚合两个独立接口产品，key 不同**，天气 key 走 `local.properties` 的 `JUHE_WEATHER_API_KEY` → BuildConfig（新闻用 `JUHE_NEWS_API_KEY`），均不入库；实况+近5天预报+生活指数，天气卡随档位切实景照片背景，默认杭州可切换城市，DataStore 持久化选定城市。
...
- **base/**：`BaseActivity<VB>`、`BaseFragment<VB>`（onDestroyView 自动释放 binding）、`BaseViewModel`（`launchWithState`/`launchListWithState` 帮手 + 统一 `errorEvent`）、`BaseApplication`（onCreate 初始化日志/崩溃捕获/前后台监听，`companion object instance`）、`BaseConstant`（常量 object，`BASE_URL`）。
...
- **NetworkModule**（Hilt `@Module`/`SingletonComponent`）：`@Provides` Gson / OkHttpClient / Retrofit / CookieJar / TokenProvider。15s 超时、失败重连、接入 `ApiDns`、`CookieManager`、Header/Token 拦截器、DEBUG 日志拦截器。要多 baseUrl 用 `@Qualifier`。
...
- **cookie/**：`CookieManager`(CookieJar) + `PersistentCookieStore` + `OkHttpCookies`，持久化到 SharedPreferences，已接入 OkHttpClient。
...
**module_news**
- `data/source/`：`NewsDataSource`(接口)、`FakeNewsDataSource`(假数据+delay，供单测与无 key 演示)、`RemoteNewsDataSource`(真实接口，`JuheResponse.unwrap`)、`NewsApi`(Retrofit suspend 接口，`@NewsRetrofit`)。
...
**module_weather**
- `data/source/`：`WeatherDataSource`(接口)、`FakeWeatherDataSource`(假数据+delay)、`RemoteWeatherDataSource`(真实接口，`JuheResponse.unwrap`)、`WeatherApi`(Retrofit suspend，`@WeatherRetrofit`)、`JuheResponse`(通用聚合响应壳)、`WeatherDto`(实况/预报/生活指数 DTO)。
...
- `data/model/WeatherCategory`：wid → 10 档映射（CLEAR/CLOUDY/LIGHT_RAIN/STORM_RAIN/THUNDER/LIGHT_SNOW/HEAVY_SNOW/SLEET/FOG/DUST，未知兜底 CLOUDY）。
- `ui/WeatherCategoryRes`：档位 → 天气卡实景照片背景（`drawable-nodpi/w_*.png` 10 张，切自单张九宫格图）+ 矢量小图标（复用 5 个基础插画）。
- `di/WeatherModule`：Hilt `@Module`，独立 `@WeatherRetrofit` Retrofit，baseUrl `https://apis.juhe.cn/`。
- `ui/`：`WeatherViewModel`(`@HiltViewModel`)、`WeatherFragment`(SwipeRefresh + StateLayout；天气卡为实景照片 centerCrop + 半透明遮罩 + 白字，顶部城市栏在 StateLayout 外始终可切换)、`ForecastAdapter`(横向近5天)、`LifeAdapter`(网格生活指数，点击弹详情)。
