...
- **module_news** (`com.btg.news`)：新闻。聚合数据接口（`JuheResponse.unwrap` → safeApiCall 体系，`@JuheRetrofit` 独立 baseUrl `https://v.juhe.cn/`）；API key 走 `local.properties` 的 `JUHE_NEWS_API_KEY` → BuildConfig，不入库；TabLayout 10 分类 + 分页 + SwipeRefresh + StateLayout + 详情 WebView。
