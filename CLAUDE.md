...
- **module_weather** (`com.btg.weather`)：天气。聚合数据接口（独立 `@WeatherJuheRetrofit`，baseUrl `https://apis.juhe.cn/`）；**天气与新闻是聚合两个独立接口产品，key 不同**，天气 key 走 `local.properties` 的 `JUHE_WEATHER_API_KEY` → BuildConfig（新闻用 `JUHE_NEWS_API_KEY`），均不入库；实况+近5天预报+生活指数，背景随档位渐变+矢量插画，默认杭州可切换城市，DataStore 持久化选定城市。
